...
sys.argv = ['detection.py', '-s', '--vis_thres', '0.6', '--cpu']
...
def detection(capture, net, device):
    net = net.to(device)
...
if __name__ == "__main__":
    ...
    while True:
        t1, img_raw, dets = detection(capture, net, device)
        # show boxed image
        for b in dets:
        ...
        cv2.waitKey(1)
        t2 = time.time()
        print("fps: %f" % (1/(t2-t1))) # t2-t1 is second per frame -> convert to fps
